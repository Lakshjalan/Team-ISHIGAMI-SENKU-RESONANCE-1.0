...
import { AppScreen, ConflictItem, GoldenRecord, RunRecord, AuditLogEntry, SourceRegistry } from './types';
...
export const App: React.FC = () => {
  const [currentScreen, setCurrentScreen] = useState<AppScreen>('command');
  ...
  const [goldenRecords] = useState<GoldenRecord[]>([
    {
      id: 'GOLD-01',
  ...
  const [undoBuffer, setUndoBuffer] = useState<ConflictItem | null>(null);
  const [isSimulatingRun, setIsSimulatingRun] = useState(false);

  const triggerToast = (msg: string, reversible?: ConflictItem) => {
    setToastMessage(msg);
  ...
  const handleResolveConflict = (conflictId: string, chosenValue: string, sourceName: string, rememberRule: boolean) => {
  ...
  const handleBatchResolve = (ids: string[]) => {
