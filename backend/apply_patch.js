...
const oldBlock =           // Triage Classification
...
const newBlock =           // Triage Classification
          if (confidenceScore >= 0.90) {
...
const uniqueLinkStr =         await supabase.from('student_entity_links').insert([
          { raw_student_id: newRec.id, master_student_id: master.id, match_score: 1.0, link_type: 'UNIQUE_CREATION' }
        ]);;
const newUniqueLinkStr =         await supabase.from('student_entity_links').insert([
          { raw_student_id: newRec.id, master_student_id: master.id, match_score: 1.0, link_type: 'INITIAL_INGESTION' }
        ]);;
if (content.includes(uniqueLinkStr)) {
  content = content.replace(uniqueLinkStr, newUniqueLinkStr);
} else {
  console.log('Failed to find uniqueLinkStr');
}
